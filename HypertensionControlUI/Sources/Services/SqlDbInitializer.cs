...
{
    internal class SqlDbInitializer : CreateDatabaseIfNotExists<SqlDbContext>
    {
        #region Public methods

        public override void InitializeDatabase(SqlDbContext context)
        {
            var dbPath = Path.Combine( AppDomain.CurrentDomain.BaseDirectory, "hypertensiondb.mdf" );

            if ( File.Exists( dbPath ) )
                return;

            var dbResourceStream = typeof (App).Assembly.GetManifestResourceStream( "HypertensionControlUI.Resources.hypertensiondb.mdf" );
            if ( dbResourceStream == null )
                throw new Exception( "Missing embedded database resource" );

            using ( var stream = File.OpenWrite( dbPath ) )
                dbResourceStream.CopyTo( stream );
        ...
        protected override void Seed(SqlDbContext context )
        {
            ...
            OptimalCutOffCalculator optimalCutOffCalculator = new OptimalCutOffCalculator(new PatientPropertyProvider());
        ...
        {
            //  Call once!
            const string filePath = @"C:\Users\Asaniel\Documents\last base.csv";
